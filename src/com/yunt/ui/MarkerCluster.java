...
public class MarkerCluster {
	 ...
	 *            区域大小参数
	 */
	public MarkerCluster(Activity activity, MarkerOptions firstMarkers, Projection projection, int gridSize) {
	...
	public void setpositionAndIcon() {
		...
		switch (iconType) {
		case 0:
			options.icon(BitmapDescriptorFactory.fromBitmap(getViewBitmap(getView(size, R.drawable.ub__pin_eta_destination))));
			break;
		case 1:
			options.icon(BitmapDescriptorFactory.fromBitmap(getViewBitmap(getView(size, R.drawable.ub__pin_eta_destination))));
			break;
