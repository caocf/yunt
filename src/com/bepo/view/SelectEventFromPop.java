 ...
 * @author simas
 * @describe 事件来源的弹出pop
 * @time 2015-5-11 08:55:45
 * 
 */

public class SelectEventFromPop extends PopupWindow {
	...
	public SelectEventFromPop(Activity context, OnClickListener itemsOnClick) {
		super(context);
		...
		lvFrom.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, android.view.View arg1, int position, long arg3) {
				...
				mEventParameter.setEventFromMap(metaData.get(position));
	...
	private void initData() {
		...
		if (PathConfig.fromJson.equals("")) {
			metaData.clear();
			firstData.clear();
			String url = PathConfig.ADDRESS + "/gsm/sys/sysdic/queryManageTree?name_dic=CODE_EVENT_FROM";
			...
			metaData.clear();
			firstData.clear();
			metaData = JSON.parseObject(PathConfig.fromJson, new TypeReference<ArrayList<Map<String, String>>>() {
			});
