...
public class PlateAdapter extends CustomAdapter {
	...
	public PlateAdapter(ArrayList<HashMap<String, String>> data, Context context) {
		super(data, context);
	}

	@SuppressWarnings("unchecked")
	public PlateAdapter(ArrayList<HashMap<String, String>> data, ListView listView, Context context) {
		super(data, context);
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		rowView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (ChoosePlate.isBusiness) {
					// 更新详情页的车牌号
					ParkDetailAct2b.setPlate(data.get(position).get("PLATE1") + data.get(position).get("PLATE2")
							+ data.get(position).get("PLATE3"));
				} else {
					// 更新详情页的车牌号
					ParkDetailAct2.setPlate(data.get(position).get("PLATE1") + data.get(position).get("PLATE2")
							+ data.get(position).get("PLATE3"));
				}

				// 更新后台数据
				ChoosePlate.confirmPlate(data.get(position).get("CODE").toString());
